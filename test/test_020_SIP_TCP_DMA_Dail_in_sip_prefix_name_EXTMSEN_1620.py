...
class testdialindmaformep(unittest.TestCase):

    def test_dial_in_dma_MS_sip_tcp_form_ep(self):
        ...
        try:
            ...
            with open('output.txt', 'a') as f:
                f.write("\n")
            try:
                assert pertext == "SIP", "EXTMSEN-1620-Assert error: Test case is failed because protocol is not SIP!"
            ...
            if createdtime >= dt_string and pertext == 'SIP':
                ...
                print("failed")
            assert createdtime >= dt_string, "EXTMSEN-1620-Assert error: file is not created"
            self.assertEqual(pertext, 'SIP',"EXTMSEN-1620-Assert error: Test case is failed because protocol is not SIP!")
        finally:
            try:
