...
class Dialtoepsip_UDP_prefixname(unittest.TestCase):

    def test_dial_out_SIP_UDP_EP_Prefixname(self):
        try:
            print("Hello....")
            parser = ConfigParser()
            ...
            with open('output.txt', 'a') as f:
                f.write("\n")
            try:
                assert pertext == "SIP", "EXTMSEN-1628-Assert error: Test case is failed because protocol is not H323!"
            ...
            if pertext == 'SIP':
                with open('output.txt', 'a') as f:
                    f.write("\n")
                    f.write('EXTMSEN-1618-Test case is passed')
                    f.write("\n")
            ...
            assert createdtime >= dt_string, "EXTMSEN-1618-Assert error: file is not created"
            self.assertEqual(pertext, 'SIP',"EXTMSEN-1618-Assert error: Test case is failed because protocol is not H323!")
        finally:
            try:
