...
class Dialtoepsip_prefixname(unittest.TestCase):
    ...
    def test_dial_out_TCP_SIP_EP_Prefixname(self):
        ...
        try:
            ...
            if pertext == 'SIP':
                with open('output.txt', 'a') as f:
                    f.write("\n")
                    f.write('EXTMSEN-1617-Test case is passed')
                    f.write("\n")
            ...
            assert createdtime >= dt_string, "EXTMSEN-1617-Assert error: file is not created"
            self.assertEqual(pertext, 'SIP',"EXTMSEN-1617-Assert error: Test case is failed because protocol is not SIP!")
        finally:
            try:
