...
from .Login_config.RMX_Manager_H323 import RMXmanagerwithH323startrecording
...
class Dialoutrecordinglink(unittest.TestCase):
    ...
    def test_dialoutfromRMX(self):
        ...
        #except Exception as e:
            ...
            #myepcall = Eplogin(parser.get('bug_tracker', 'epurl'), driver)  # Login the EP
            #myepcall.doepLog35  # EP will dial the conference
        now = datetime.now()
        ...
        rmxh323recordinglogin = RMXmanagerwithH323startrecording(pyautogui)
        rmxh323recordinglogin.RMXmanagerH323()
        time.sleep(5)
        ...
        with open('output.txt', 'a') as f:
            f.write("\n")
        try:
            assert pertext == "H323", "EXTMSEN-1630-Assert error: Test case is failed because protocol is not H323!"
        ...
        if createdtime >= dt_string and pertext == 'H323':
            ...
            sheet.cell(row=9, column=3).value = 'Pass'
            print('value is', sheet.cell(row=9, column=3).value)
            ...
            sheet = book['sample']
            print('sheet is', sheet)
            sheet.cell(row=9, column=3).value = 'Fail'
            print('value is', sheet.cell(row=9, column=3).value)
            book.save('C:\\Users\\njain\\Desktop\\Report.xlsx')

        assert createdtime >= dt_string, "EXTMSEN-1630-Assert error: file is not created"
        self.assertEqual(pertext, 'H323',"EXTMSEN-1630-Assert error: Test case is failed because protocol is not H323!")
