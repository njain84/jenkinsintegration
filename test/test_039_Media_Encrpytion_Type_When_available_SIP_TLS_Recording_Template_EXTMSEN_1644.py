...
class AES(unittest.TestCase):
    ...
    def test_AES(self):
        try:
            ...
            time.sleep(2)
            print('test2')
            Encryption_call_setting.select_by_value('0')
            print('test3')
            time.sleep(2)
            ...
            time.sleep(3)
            self.select1 = Select(
                self.driver.find_element_by_xpath("//*[@id='modalForm:sel_aestype']"))  # Media encryption type
            time.sleep(2)
            self.select1.select_by_visible_text('Required for All Calls')
            time.sleep(2)
            self.driver.find_element_by_xpath(
                "//*[@id='modalForm:modalPanel_body']/div[3]/button[2]").click()  # click ok
            time.sleep(3)
            self.driver.find_element_by_xpath("//*[@id='a_1001']").click()  # click on call tab
            time.sleep(8)
            self.driver.find_element_by_xpath(
                "//*[@id='mainForm:j_idt8_body']/button[2]").click()  # click on Dial out to record
            time.sleep(3)
            self.driver.find_element_by_xpath("//*[@id='modalForm:address_p1']").send_keys(
                parser.get('bug_tracker', 'EPIP'))  # click on Address
            time.sleep(3)
            self.select1 = Select(self.driver.find_element_by_xpath("//*[@id='modalForm:j_idt8']"))
            self.select1.select_by_visible_text("SIP")  # selecting SIP
            time.sleep(2)
            ...
            time.sleep(5)
            # Change it to Encryption OFF and try SIP call
            self.driver.find_elements_by_class_name('btn')[0].click()  # click on configuration
            time.sleep(2)
            self.driver.find_element_by_id('headerForm:j_idt10:0:j_idt12').click()
            print('test')
            time.sleep(5)
            Encryption_call_setting = Select(
                self.driver.find_element_by_id("modalForm:sel_aestype"))  # Media encryption type
            time.sleep(2)
            print('OFF')
            ...
            time.sleep(3)
            self.driver.find_element_by_xpath(
                "//*[@id='rTemplate-table-row-0']/td[1]/input").click()  # Click on Default Recording
            time.sleep(3)
            self.driver.find_element_by_xpath("//*[@id='mainForm:j_idt7_body']/button[3]").click()  # click on edit
            time.sleep(3)
            self.select1 = Select(
            ...
            assert (
                        match == 'AES_256' or match == 'AES_128'), "EXTMSEN-1637-Assert error: Test case is failed because Encryption is disabled"
            ...
            try:
                ...
                time.sleep(2)
                self.driver.find_element_by_xpath(
                    "//*[@id='modalForm:modalPanel_body']/div[3]/button[2]").click()  # click
                time.sleep(3)
                self.driver.find_element_by_xpath("//*[@id='a_1001']").click()  # click on call tab
                time.sleep(3)
