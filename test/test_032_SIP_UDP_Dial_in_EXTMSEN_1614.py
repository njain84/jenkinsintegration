...
class admin_udp_dialin_call_without_DMA(unittest.TestCase):

    def test_SIP_UDP_Dialin_call(self):
        ...
        try:
            ...
            # UDP will be setting up
            udp_protocol_setting = admin_Registration(driver)
            actualprotocolvalue = udp_protocol_setting.admin_registration_udp_check()
            print('Value is:',actualprotocolvalue)
            # Ep will be registratering with GK
            ...
            assert actualresult[0] >= callstart_time, "EXTMSEN-1614-Assert error: file is not created"
            assert 'SIP' == actualresult[1], "EXTMSEN-1614_Test case is failed because protocol is not SIP!"
            assert actualprotocolvalue == '0', "EXTMSEN-1614_Test case is failed because transport protocol is not UDP!"
