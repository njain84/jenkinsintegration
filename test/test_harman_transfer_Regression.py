...
class Transfer_harman(unittest.TestCase):
    ...
    def test_report_transfer(self):
        ...
        self.driver.find_element_by_id('message_subject').send_keys('Regression result ' + (parser.get('bug_tracker', 'build')))
        time.sleep(5)
        pyautogui.moveTo(550, 843)     #click on message body
        pyautogui.click()
        time.sleep(5)
        pyautogui.write('Hi All,')
        pyautogui.press('enter')  # press the Enter key
        pyautogui.write('Please find the attached regression test report of Build ' + parser.get('bug_tracker', 'build'))
        pyautogui.press('enter')  # press the Enter key
        ...
        time.sleep(5)
        pyautogui.write('C:/Jenkins/workspace/Regression_report')
        pyautogui.press('enter')  #press the Enter key
        time.sleep(3)
